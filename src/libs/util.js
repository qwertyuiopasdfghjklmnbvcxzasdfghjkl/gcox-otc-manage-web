...
// const ajaxUrl = env === 'production' ? `${location.protocol}//${location.host}/` : 'http://manage.contract-king.koall.io/';
...
const systemInfosEN = {
    BUYER: 'buyers',
    OTC_CANCEL_ORDER: 'The trading order has been cancelled.',
    OTC_CANCEL_ORDER_BODY: 'The trading order has been cancelled.',
    OTC_NEW_ORDER: 'Your OTC has a new order, please go to process it.',
    OTC_NEW_ORDER_BODY: 'Your OTC has a new order, please go to process it.',
    OTC_USER_UNFROZEN: 'User defrosted',
    OTC_USER_UNFROZEN_BODY: 'You have been defrosted for new OTC transactions',
    SELLER: 'seller',
    OTC_PAY_FINISH: 'The payment has been marked to confirm, please check.',
    OTC_PAY_FINISH_BODY: 'The payment has been marked to confirm, please check.',
    OTC_RELEASE_SYMBOL: 'The payment has been confirmed, please check.',
    OTC_RELEASE_SYMBOL_BODY: 'The payment has been confirmed, please check.',
    OTC_SYSTEM_CANCEL_ORDER: 'The order timeout has not been processed and has been cancelled automatically.',
    OTC_SYSTEM_CANCEL_ORDER_BODY: 'The order timeout has not been processed and has been cancelled automatically.',
    OTC_USER_FROZEN: 'The system has suspended your new OTC transaction for 24 hours because you cancelled too many orders.',
    OTC_USER_FROZEN_BODY: 'The system has suspended your new OTC transaction for 24 hours because you cancelled too many orders.',
    OTC_FROCE_RELEASE_SYMBOL: 'COINS have been released compulsorily',
    OTC_FROCE_RELEASE_SYMBOL_BODY: 'COINS have been released compulsorily',
    OTC_UNLOCK_ORDER: 'Unlocked currency',
    OTC_UNLOCK_ORDER_BODY: 'Unlocked currency',
    EXPORT_OTC_FAIL: 'Export report failed',
    EXPORT_OTC_SUCC: 'Report exported successfully'
};
...
const formatSystemMessage = function (msg, isI18n) {
    ...
    msg = (msg || '').split(' ');
    let joinSplit = ' ';
    let lang = window.localStorage.language === 'en-US';
    if (msg.length === 1) {
        joinSplit = '';
        msg[0] = (lang ? systemInfosEN[msg[0]] : systemInfos[msg[0]]) || msg[0];
    } else if (msg.length === 2) {
        msg[1] = (lang ? systemInfosEN[msg[1]] : systemInfos[msg[1]]) || msg[1];
    } else if (msg.length === 3) {
        msg[0] = (lang ? systemInfosEN[msg[0]] : systemInfos[msg[0]]) || msg[0];
        msg[2] = (lang ? systemInfosEN[msg[2]] : systemInfos[msg[2]]) || msg[2];
    }
