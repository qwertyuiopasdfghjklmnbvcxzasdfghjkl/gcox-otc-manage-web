...
// 分页查询KYC保证金
const listPageUserKycDeposit = function (size, page, data, success, error) {
  api.post(`api/bm/kycManage/listPageUserKycDeposit/${size}/${page}`, data, (res) => {
    if (res.rst === 1) {
      success && success(res.data, res.total)
    } else {
      error && error(res.msg)
...
kyc.listPageUserKycDeposit = listPageUserKycDeposit

// 更新KYC保证金
const updateUserKycDeposit = function (data, success, error) {
  api.post('api/bm/kycManage/updateUserKycDeposit', data, (res) => {
    if (res.rst === 1) {
...
kyc.updateUserKycDeposit = updateUserKycDeposit
